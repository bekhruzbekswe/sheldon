# synthesis-orchestrator Specification (delta)
...
3. **If thesis-drafting succeeds** (returns a non-null result with ≥3 claims AND `factStore.count() >= 30`): proceed with the thesis path (D2 below).
4. **Otherwise**: emit `synthesis.fallback` with `payload: {reason: 'thesis_failed' | 'thesis_returned_null' | 'sparse_corpus', factCount, validClaims?}` and proceed with the S1 cluster path (call `clusterFacts(...)` and write per cluster as before).

The cluster fallback path MUST use the S1-improved section-writer (existing `writeSection(input, ctx)` function) so prompt-level S1 wins are preserved when fallback fires.

`MIN_FACTS_FOR_THESIS` is a per-file constant in `src/synthesize.ts` with starting value `30`.
...
4. **Brutal-editor pass** (D3 below).
5. **Stitch**: call `stitchReport` with the surviving sections, the thesis text, and per-claim metadata for intro/outro.

If a claim's top-K retrieved facts have `cosine(fact, claim) < 0.30` for ≥80% of them, the claim is *downgraded*: emitted with a warning event and either written with explicit thin-evidence framing (default) or dropped before writing (when `MIN_CLAIM_SUPPORT === 'drop'`). Per-file constant `MIN_CLAIM_SUPPORT_MODE = 'hedge'`.
