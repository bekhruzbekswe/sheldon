# thesis-drafter Specification (delta)
...
1. Sort all facts by `cosine(fact_embedding, task_embedding)` DESCENDING. Task embedding comes from `getTaskEmbedding()` (S1's `research-contract` capability).
