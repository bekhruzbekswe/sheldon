# frontier-queue Specification

## Purpose

Persistent SQLite-backed priority queue of research questions, each scored and tagged with parent/depth/embedding metadata. Push is novelty-aware (cosine ≥ 0.85 dedupes); pop atomically transitions the highest-scored pending row to in-progress. Lives in the same `.sheldon/sheldon.db` as the fact store so future resume (L7) reopens one file. The agent's self-directed to-do list.
## Requirements
