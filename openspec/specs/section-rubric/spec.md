# section-rubric Specification

## Purpose

Per-section quality check. After a section is drafted, one `llm.fast` call evaluates it against four boolean fields — does it explain a mechanism, name a concrete example, contain quantitative signal, and actually defend its heading — plus a one-line note used as feedback for revision. Drives the brutal-editor pass in the synthesis-orchestrator (revise once on failure, then drop or flag depending on `BRUTAL_EDIT_MODE`). Fails open: a flaky rubric LLM never deletes legitimate sections.

## Requirements

### Requirement: Evaluate a written section against a structural rubric

The system SHALL provide an `evaluateSection(body, claim)` function in `src/rubric.ts` that takes a written section body and the claim it was supposed to defend, and returns:
...
The function MUST emit one `section.rubric` event with `layer:'L6'`, `durationMs`, payload `{headline, hasMechanism, hasExample, hasQuantification, defendsHeading}`.
...
If the rubric LLM call fails after retry, the function SHALL return `{hasMechanism: true, hasExample: true, hasQuantification: true, defendsHeading: true, note: 'rubric LLM failed; pass-by-default'}`. This deliberately fails-open: a broken rubric must not delete legitimate sections.
