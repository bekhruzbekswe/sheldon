# cluster-facts Specification

## Purpose

K-means clustering over the fact store's L2-normalized 384-dim embeddings. Picks `k` automatically via silhouette score across [5, 10], drops clusters smaller than `minSize` (default 5), and labels each surviving cluster from its dominant `topicTag` (or via `llm.fast` when no tag dominates). Returns clusters sorted by size descending. Used by L6 synthesis to partition the knowledge base into themes that fit one-section-per-cluster.
