# searxng-client Specification

## Purpose

Thin HTTP client for a SearXNG instance. Sends queries, parses JSON responses, returns ranked `{title, url, snippet}[]` to higher layers. The agent's only direct interface to the open web's index.
