...
Phase-boundary "what's missing?" pass. At breadth→depth (and at depth→synthesis when the frontier is still alive), one `llm.fast` call receives the run's task, contract, and a sampled cross-section of facts, then returns 3-5 concrete searchable angles the corpus is missing. The returned questions are pushed onto the frontier as fresh seeds (depth=0). Lives in `src/gap.ts`.
...
5. For each entry: embed the question, push to the frontier with `score = score({relevance, novelty, depth: 0, phase: currentPhase()})` and `parentId: undefined`. Frontier dedupe (cosine ≥ 0.85) applies as normal. **Track per-gap whether it was pushed or deduped.**
6. Emit one `gap.analyzed` event with `layer: 'L4'`, `durationMs`, payload `{phaseTransition: 'breadth->depth' | 'depth->synthesis', gapsProposed, gapsPushed, gapsDeduped, gaps: Array<{question, relevance, why, pushed}>, error?}`. **The `gaps` array is REQUIRED on the success path** (carries the actual proposed questions and per-gap dispositions for diagnostic use); on the failure path the `error` field is populated and `gaps` MAY be omitted or empty.
...
- **GIVEN** a successful gap analysis pass that proposed 4 gaps, of which 3 were pushed and 1 was deduped against an existing frontier row
...
- **AND** the deduped entry has `pushed: false`
- **AND** the three pushed entries have `pushed: true`
