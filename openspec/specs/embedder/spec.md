...
In-process sentence embeddings via `@xenova/transformers` running `Xenova/all-MiniLM-L6-v2` (384-dim, L2-normalized). Lazy single-load model cached across calls; batched at ≤32 inputs per forward pass. Powers chunk reranking (later layers) and cross-fact dedupe in the fact store.
